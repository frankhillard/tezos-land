...
class landContractTest(TestCase):
    ...
    def test_buy_owneroperator(self):
        result = self.nftContract.buyLand(1).with_amount(Decimal(0.0001)).result(
            storage={"market": {
                                "lands": {},
                                "admin": "tz1ibMpWS6n6MJn73nQHtK5f4ogyYC1z9T9z",
                                "height": 10, "width": 10,
                                "to_sell": {1: Decimal(0.0001)}},
                        "ledger": {1: alice},
        ...
        # self.assertEqual(False, (alice, self.nftContract.address, 1) in result.big_map_diff['operators'].keys())
        # verify bob balance has decreased
        # verify alice balance has increased
        # verify contract balance is the same
